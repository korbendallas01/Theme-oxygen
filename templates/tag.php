...
<koken:load infinite="true" limit="6">
	<koken:head>
		<koken:not empty="tag.title">
			<title>{{ language.viewing_everything_in_title collate='tag.title' }} - {{ site.title }}</title>
		<koken:else>
			<title>{{ language.no_items_were_found }} - {{ site.title }}</title>
		</koken:not>
		<meta property="og:site_name" content="{{ site.title }}" />
		<meta property="og:title" content="{{ language.viewing_everything_in_title collate='tag.title' }} - {{ site.title }}" />
