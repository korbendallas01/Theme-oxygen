...
<koken:load>
	<koken:head>
		...
		<meta property="og:title" content="{{ language.categories }}" />
		<meta property="og:description" content="{{ site.description strip_html='true' }}" />
		<meta property="og:type" content="website" />
		<meta property="og:url" content="{{ location.site_url }}{{ location.here }}" />
		<title>{{ language.categories }} - {{ site.title }}</title>
	</koken:head>
</koken:load>
<main role="main">
<koken:load limit="{{ settings.item_load_limits }}" infinite="true">
	<koken:if data="settings.cover_styles" equals="image_cover">
		<koken:load source="contents" filter:categories="{{ category.title }}" limit="1">
			<koken:first>
	...
	<koken:if data="settings.cover_styles" equals="text_cover">
		<article class="top-section">
			<koken:load><h1 class="page-name">{{ language.categories }}</h1></koken:load>
		...
		<koken:loop>
			<li id="content-{{ id }}">
				...
				<h3 class="name_overlay tag-cat--parent">
					<span class="tag-cat--title">{{ category.title }}</span>
					<span class="tag-cat--nr">&nbsp;x {{ category.counts.total }}</span>
				</h3>
			</li>
		</koken:loop>
	</ul>
